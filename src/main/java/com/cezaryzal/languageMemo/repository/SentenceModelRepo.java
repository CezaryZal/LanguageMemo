...
public interface SentenceModelRepo extends JpaRepository<SentenceModel, Long> {
    ...
    @Query(value = "select * from sentence where replay_level_to_native <= :limitReplayLevel", nativeQuery = true)
    ...
    @Query(value = "select * from sentence where replay_date_to_native <= :localDate order by rand() limit 1",
            nativeQuery = true)
    ...
    @Query(value = "select count(*) from sentence where replay_date_to_native <= :localDate",
            nativeQuery = true)
